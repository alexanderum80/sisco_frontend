...
export class ListCompararValoresComponent implements OnInit, OnDestroy {
  ...
  private _getCompararValores(): void {
    try {
      this._compararValoresSvc.subscription.push(this._compararValoresSvc.loadAll().subscribe(response => {
  ...
  private _add(): void {
    try {
      ...
      this._dinamicDialogSvc.open('Agregar Comparación de Valores', CompararValoresFormComponent);
      this._compararValoresSvc.subscription.push(this._dinamicDialogSvc.ref.onClose.subscribe((message: string) => {
        if (message) {
  ...
  private _edit(data: any): void {
    try {
      ...
      this._compararValoresSvc.inicializarFg();
      this._compararValoresSvc.subscription.push(this._compararValoresSvc.loadOne(data.Id).subscribe(response => {
        ...
        this._dinamicDialogSvc.open('Editar Comparación de Valores', CompararValoresFormComponent);
        this._compararValoresSvc.subscription.push(this._dinamicDialogSvc.ref.onClose.subscribe((message: string) => {
          if (message) {
            this._msgSvc.add({ severity: 'success', summary: 'Satisfactorio', detail: message })
          }
    ...
    try {
      this._sweetAlertSvc.question('¿Desea Eliminar la(s) Comparación(es) seleccionada(s)?').then(res => {
        if (res === ActionClicked.Yes) {
          ...
          this._compararValoresSvc.subscription.push(this._compararValoresSvc.delete(IDsToRemove).subscribe(response => {
