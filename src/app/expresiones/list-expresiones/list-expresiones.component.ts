...
export class ListExpresionesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private _getExpresionesResumen(): void {
    try {
      this._expresionesSvc.subscription.push(this._expresionesSvc.loadAllExpresionesResumen().subscribe(response => {
  ...
  private _add(): void {
    try {
      ...
      this._expresionesSvc.subscription.push(this._dinamicDialogSvc.ref.onClose.subscribe((message: string) => {
        if (message) {
  ...
  private _edit(data: any): void {
    try {
      if (this._usuarioSvc.usuario.IdDivision !== 100 && this._usuarioSvc.usuario.IdTipoUsuario !== ETipoUsuarios['Usuario Avanzado'] && data.Centralizada) {
        return this._sweetAlertSvc.warning('No tiene permisos para modificar una Expresión Centralizada.');
      }
    ...
    try {
      this._sweetAlertSvc.question('¿Desea Eliminar la(s) Expresión(es) seleccionada(s)?').then(res => {
        if (res === ActionClicked.Yes) {
          ...
          const IDsToRemove: number[] = data.map((d: { IdExpresion: number }) => { return d.IdExpresion });
