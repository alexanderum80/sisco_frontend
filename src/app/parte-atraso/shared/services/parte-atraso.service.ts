...
export class ParteAtrasoService {
  ...
  public async getDetalleParteAtrasoDefinition(
    detalleAtrasosData: any
  ): Promise<any> {
    ...
    unidades.forEach((u: any) => {
      ...
      const data = detalleAtrasosData.filter(
        (f: { IdUnidad: string }) => f.IdUnidad === u.IdUnidad
      );
